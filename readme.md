...
## Перенаправление Syslog сообщений
Если необходимо перенаправлять Syslog сообщения также на другой Syslog-сервер, то нужно использовать фукнцию:

```javascript
syslog.setForward(ip, port);
```
Для влкючения и выключения перенаправления сообщений нужно использовать функции **enableForward** и **disableForward**.
